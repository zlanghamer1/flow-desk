# Guardrail history — flow-desk

This file is the full, unedited text of `CLAUDE.md` as it stood on 2026-09-03,
before that file was rewritten as a distilled rulebook. It is a record, not
active direction.

**Read the rulebook in `CLAUDE.md` first.** Come here when you need the *why*
behind a rule: the live measurement that set a constant, the bug a guard was
written against, the reasoning a past pass rejected. Every standing rule below
also appears in `CLAUDE.md`; if the two ever disagree, `CLAUDE.md` wins and
this file is stale.

Per-finding tables and scores for every review round live in
`docs/OPEN_ITEMS.md`.
