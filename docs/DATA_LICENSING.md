...
| Yahoo Finance `wss://streamer.finance.yahoo.com/?version=2` (added 2026-09-23) | Real-time last trade for the one name open in the Day trade tab, plus SPY as the heartbeat that shows the stream is alive. | The page, only while that tab is open on the chart view | Unofficial and undocumented; no key, no Origin check. Same Yahoo terms as the row above: personal, non-commercial use, no redistribution. | **Blocked.** |
...
| ICI `www.ici.org/research/stats/combined_flows` | US fund flows card: weekly estimated net flows for all US long-term mutual funds and ETFs (domestic equity, world equity, hybrid, bond, commodity). | Fetcher, hourly gate; the page itself changes once a week | Public statistics page, no key, no login. The request carries a browser navigation header set because the site's edge answers 403 to a bare GET (measured 2026-09-12). The site's terms page was not located from the statistics page; ICI's releases are press releases and cite themselves as the source. | **Verify.** Ask ICI in writing before any redistribution beyond this personal page; cite ICI on the page (the honesty box does). |
